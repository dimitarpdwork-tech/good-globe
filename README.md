# The Good Globe — a fully automated good-news website

Only good news. An AI editor reads trusted sources several times a day, keeps
**only** the genuinely positive stories, writes original summaries in English
...
- `pipeline.py` — fetches feeds, asks Claude to select only positive stories and
  write original English summaries (never invented facts, never copied text).
...
1. **Create a GitHub repository** (e.g. `good-globe`) and push this folder to it:

       git init && git add -A && git commit -m "launch"
       git branch -M main
       git remote add origin https://github.com/YOURNAME/good-globe.git
...
5. **Set your real address.** In `config.json` change `base_url` to your final
   ...
   `https://YOURNAME.github.io/good-globe` — and also set `base_path` to
   `/good-globe` in that case. With a custom domain, `base_path` stays `""`.

### Custom domain (recommended for SEO)

Buy a domain (e.g. `goodglobe.news`), then: repo *Settings → Pages → Custom domain*,
